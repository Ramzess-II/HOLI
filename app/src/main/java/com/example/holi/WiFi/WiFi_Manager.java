...
public class WiFi_Manager extends Thread {   // поток для создания сокета
    public boolean more_connect = false;
    public boolean connect_ok = false;
    private Context context;
    private Socket socket;
    private String _ip, _port;
    private WiFi_Transmitter wifiTransmiter;

    public WiFi_Manager(Context context, String ip, String port) {   // конструктор, сюда передаем ip и порт
        this.context = context;
        _ip = ip;
        _port = port;
        wifiTransmiter = new WiFi_Transmitter(context);               // создаем класс передатчика
    }
    ...
    public WiFi_Transmitter probros() {
